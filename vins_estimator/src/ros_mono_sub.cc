...
int main(int argc, char **argv){
	...
	//地图左下角对应的物理坐标
	grid_map_msg.info.origin.position.x = grid_min_x;
	grid_map_msg.info.origin.position.y = grid_min_z;
	...
	ros::Subscriber sub_pts_and_pose = nodeHandler.subscribe("/vins_estimator/kf_and_mps", 1000, ptCallback);
...
void updateGridMap(const geometry_msgs::PoseArray::ConstPtr& pts_and_pose){
	...
	ROS_INFO("发送的当前帧相机位姿Y:%f",kf_location.z);
