...
// 声明map and read param
void PoseGraph::readGridParametersAndInitMap(ros::NodeHandle &n)
{  
    ...
    canny_detection = fsSettings["canny_detection"];

    ROS_WARN("loop map param read");
    fsSettings.release();
...
void PoseGraph::registerPub(ros::NodeHandle &n)
{
	...
	pub_grid_map = n.advertise<nav_msgs::OccupancyGrid>("grid_map", 1000); // grid pub
    for (int i = 1; i < 10; i++)
...
void PoseGraph::updatePath()
{
    ...
    kf_pt_array.poses.push_back(geometry_msgs::Pose());// 占坑
    ...
    for (it = keyframelist.begin(); it != keyframelist.end(); it++)
    {
        ...
        // 更新栅格信息
        kf_pt_array.header.frame_id = "loop_map";
        geometry_msgs::Pose n_kf_msg;
	    n_kf_msg.position.x = n_kf_msg.position.y = n_kf_msg.position.z = keyframelist.size();
        // // 发现问题拉：poses[0] 没有值，不能直接引用填充。必须要push_back一个值，然后才能替换/。
        kf_pt_array.poses[0] = n_kf_msg; // 第一个位置存的kf的数量
        // 将关键帧的位置送入kf_pt_array
	    geometry_msgs::Pose kf_pose;
        kf_pose.position.x =P.x();
        kf_pose.position.y =P.y();
        kf_pose.position.z =P.z();
	    kf_pose.orientation.x = Q.x();
		kf_pose.orientation.y = Q.y();
		kf_pose.orientation.z = Q.z();
		kf_pose.orientation.w = Q.w();
		kf_pt_array.poses.push_back(kf_pose);
        // 将关键帧所对应点云送入kf_pt_array
		unsigned int n_pts_id = kf_pt_array.poses.size();
        //占个坑，一会填入pt的个数  placeholder for number of points
        kf_pt_array.poses.push_back(geometry_msgs::Pose());
        std::vector<cv::Point3f> map_points = (*it)->point_3d;
		unsigned int n_pts = 0;
        for (auto map_pt : map_points)
        {
            if(sqrtf((P.x()-map_pt.x)*(P.x()-map_pt.x)+(P.y()-map_pt.y)*(P.y()-map_pt.y)) < 5.0){
                // 连线的距离小于5m，认为是有效点
                geometry_msgs::Pose curr_pt;
                curr_pt.position.x = map_pt.x;
				curr_pt.position.y = map_pt.y;
				curr_pt.position.z = map_pt.z;
    ...
    // 装载完毕。开始地图更新
    ROS_WARN("map loading  over....");
    pubLoopKeyframeMap(kf_pt_array);
    publish();

    m_keyframelist.unlock();
}
void PoseGraph::pubLoopKeyframeMap(const geometry_msgs::PoseArray & all_kf_and_pts){

	resetGridMap(all_kf_and_pts);
}
void PoseGraph::resetGridMap(const geometry_msgs::PoseArray & all_kf_and_pts){
...
void PoseGraph::processMapPts(const std::vector<geometry_msgs::Pose> &pts, unsigned int n_pts,
	unsigned int start_id, int kf_pos_grid_x, int kf_pos_grid_z){

    //cout<< "回环的关键帧所对应的地图点的个数有" <<  n_pts << endl;
...
void PoseGraph::processMapPt(const geometry_msgs::Point &curr_pt, cv::Mat &occupied,
...
void  PoseGraph::getGridMap(){
        // 遍历，算概率，映射到grid_map_msg的0-100
    	for (int row = 0; row < h; ++row){
		for (int col = 0; col < w; ++col){
    ...
    if(canny_detection){
        ...
        // cv::Mat flipImg;
        int t1_value = 50;
        //对grid_map_thresh 图片资源进行canny，对检测后的图片中的白色像素对应的row和col的grid_map_int置为100（黑色）
        // cv::namedWindow("src", cv::WINDOW_AUTOSIZE);
        // cv::imshow("src", grid_map_thresh); 
        //垂直翻转 沿着X轴
        // cv::flip(grid_map_thresh,flipImg,0);

        GaussianBlur(grid_map_thresh, srcBlur, cv::Size(3, 3), 0, 0);
        cv::Mat edgeOutput;
        cv::Canny(srcBlur, edgeOutput, t1_value, t1_value*2, 3, false);
        for(int row = 0; row < h; row++){
            for(int col = 0; col < w; col++){
                //注意 uchar 范围0~255，换成 int 会强制类型转换，换成char的话，最高是128
                if(edgeOutput.at<uchar>(row,col) == 255 ){
                    grid_map_int.at<char >(row,col) = 100;
                }
            }
        }
        // cv::namedWindow("output", cv::WINDOW_AUTOSIZE);
        // cv::imshow("output", edgeOutput);
        // cv::waitKey(0); 
    }
}
