...
public:
	...
	~PoseGraph();
	//读取栅格参数
	void readGridParametersAndInitMap(ros::NodeHandle &n);
...
private:
	...
	void resetGridMap(const geometry_msgs::PoseArray & all_kf_and_pts);

    void pubLoopKeyframeMap(const geometry_msgs::PoseArray & all_kf_and_pts);    
	...
	double grid_res_x;
	double grid_res_z;
	cv::Mat global_occupied_counter;
	cv::Mat global_visit_counter;
	cv::Mat local_occupied_counter;
	cv::Mat local_visit_counter;
